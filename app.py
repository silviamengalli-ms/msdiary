...
if df_storico is not None and not df_storico.empty:
    # Pulizia righe vuote basata sulla struttura delle tue colonne
    ...
    # Estrazione dinamica delle opzioni dai tuoi menu a discesa storici
    ...
    # Recupero meteo automatico per facilitare la scelta della categoria testuale
    ...
    # --- TRASFORMAZIONE DELLE COLONNE TESTUALI IN VARIABILI MATEMATICHE ---
    ...
    if st.button("🔮 Calcola Predizione", type="primary"):
        ...
        nuovo_giorno_dummy = nuevo_giorno_dummy.reindex(columns=X_dummy.columns, fill_value=0)
        
        # Calcolo predizione
        predizione = modello.predict(nuovo_giorno_dummy)[0]
        predizione = max(1.0, min(10.0, predizione))
        
        st.success(f"### 🎯 Predizione :red[S]:orange[e]:green[m]aforo Energetico")
