...
def calcola_accumulo_72ore():
    ...
    try:
        ...
        for i, etichetta in enumerate(reversed(giorni_etichette)):
            record = ultimi_3_giorni[i]  # <-- Corretto qui (era ultime_3_giorni)
...
with tab_mattina:
    ...
    if st.button("🚀 Calcola e Salva Mattina", use_container_width=True):
        ...
        attivita_esposte = {"ufficio", "piccole commissioni", "visita", "fisioterapia", "sociale"}
        ha_attivita_esposte = any(a in attivita_esposte for a in attivita)
        ...
        if not ha_attivita_esposte and len(attivita) > 0:
            p_temp = 0.0
            stato_calore = "no"
    ...
    if not st.session_state.mattina_salvata:
        ...
        if st.button("💾 REGISTRA IL MIO DIARIO", use_container_width=True):
            ...
            payload = {
                ...
                ENTRY_ID['calore_form']: st.session_state.esposizione_reale_calore  
            }
