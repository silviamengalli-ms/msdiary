...
# FUNZIONE METEO AUTOMATICA (RESTITUISCE IL NUMERO DEI GRADI)
...
if df_storico is not None and not df_storico.empty:
    ...
    if df_pulito.empty:
        ...
        if temp_rilevata is not None:
            st.success(f"🌤️ **Meteo rilevato automaticamente a Verona:** {temp_rilevata}°C")
            ...
            st.error("❌ Impossibile rilevare il meteo automaticamente in questo momento. Controlla la connessione internet o riprova più tardi.")
else:
    st.warning("Assicurati che il Foglio Google contenga i dati iniziali.")
